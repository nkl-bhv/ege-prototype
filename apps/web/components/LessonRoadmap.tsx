  ...
  return (
    <section className="relative rounded-3xl bg-white px-5 pb-8 pt-6 shadow-card">
      ...
      <header className="mb-6 flex items-center justify-between">
        ...
        <button className="rounded-full border border-neutral-200 px-4 py-2 text-xs font-semibold text-neutral-500 transition hover:border-secondary hover:text-secondary">
          Справка
        </button>
      </header>
      <ol className="relative flex flex-col gap-6">
        {lessons.map((lesson, index) => (
          <div key={lesson.id} className="relative pl-10">
            <span className="absolute left-4 top-6 h-4 w-4 -translate-x-1/2 rounded-full border-4 border-white shadow-soft" style={{ background: nodeColor(lesson.state) }} />
